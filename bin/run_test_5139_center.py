...
if __name__ == "__main__":
    ...
    # v_sys = 232.5*u.km/u.s
    # r_peak = 30.*u.arcsec
    # a = 30.*u.arcsec
    # theta_0 =  2.094395*u.rad#2.*np.pi*np.random.random()*u.rad
    # sigma_max = (20. + 8.*np.random.random())*u.km/u.s
    ...
    center = (201.696718746, -47.479909445555)
    #     # (201.69184583, -47.47911111) #n08
    ...
    for i in range(data.data['bin'].max() + 1):
        ...
        cf.parameters['v_maxx'].set(min=-100, max=100,initials='rng.normal(loc=0, scale=2, size=n)')
        cf.parameters['v_sys'].set(value=232.5,fixed=True)
        # cf.parameters['dx'].set(min=-100,max=100,fixed=False,initials='rng.normal(loc=0,scale=1,size=n)')
        # cf.parameters['dy'].set(min=-100, max=100, fixed=False, initials='rng.normal(loc=0,scale=1,size=n)')
        cf.parameters['dx'].set(value=8, fixed=True)
        cf.parameters['dy'].set(value=-8, fixed=True)
        # cf.parameters['dx'].set(min=-1, max=1, fixed=False, initials='rng.uniform(low=-1,high=1,size=n)')
        # cf.parameters['dy'].set(min=-1, max=1, fixed=False, initials='rng.uniform(low=-1,high=1,size=n)')
        ...
        cf.parameters['v_maxy'].set(min=-100, max=100,initials='rng.normal(loc=0, scale=1, size=n)')  # , expr="v_maxx*tan(theta_0)")
        ...
        write_table.write('/Users/ljmu/Desktop/muse/output/mcmc_chains_constant_fixedvsys_n08_off.fits', format='fits', overwrite=True)
        plt.show()
    ...
    mf.parameters['v_maxx'].set(min=-100, max=100, initials='rng.normal(loc=0, scale=2, size=n)')
    ...
    mf.parameters['r_peak'].set(min=0, max=500, initials='300*rng.beta(a=2, b=5, size=n)')
    mf.parameters['v_sys'].set(value=232.5, fixed=True)
    # mf.parameters['v_sys'].set(min=0, max=400,initials='rng.normal(loc=232.5, scale=1, size=n)')
    mf.parameters['v_maxy'].set(min=-100, max=100, initials='rng.normal(loc=0, scale=1, size=n)')  # , expr="v_maxx*tan(theta_0)")
    # mf.parameters['dx'].set(min=-100, max=100, fixed=False, initials='rng.normal(loc=0, scale=1, size=n)')
    # mf.parameters['dy'].set(min=-100, max=100, fixed=False, initials='rng.normal(loc=0, scale=1, size=n)')
    mf.parameters['dx'].set(value=8, fixed=True)
    mf.parameters['dy'].set(value=-8, fixed=True)
    ...
    write_table = Table({'chain': sampler.chain, 'lnprob': sampler.lnprobability})
    write_table.write('/Users/ljmu/Desktop/muse/output/mcmc_chains_model_fixedvsys_n08_off.fits', format='fits', overwrite=True)
    ...
    radial_profile.write('/Users/ljmu/Desktop/muse/output/radial_profile_fixedvsys_n08_off.csv')
    radial_model.write('/Users/ljmu/Desktop/muse/output/radial_model_fixedvsys_n08_off.csv')
    plt.show()
